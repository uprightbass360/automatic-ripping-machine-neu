...
def get_tmdb_poster(search_query=None, year=None):
    ...
    if 'status_code' in search_results:
        app.logger.debug("get_tmdb_poster failed with status_code %s", int(search_results.get('status_code', 0)))
        return None

    # If movies are found return those after processing
    if search_results['total_results'] > 0:
        app.logger.debug("TMDB movie results: %d", int(search_results['total_results']))
    ...
    if search_results['total_results'] > 0:
        app.logger.debug("TMDB tv results: %d", int(search_results['total_results']))
        return tmdb_process_poster(search_results, poster_base)
...
def tmdb_process_poster(search_results, poster_base):
    ...
    for media in search_results['results']:
        if media['poster_path'] is not None and 'release_date' in media:
            ...
            app.logger.debug("TMDB poster match: %s (%s)", str(media['title']), str(released_date))
            ...
            media['Type'] = "movie"
            app.logger.debug("TMDB backdrop found for %s", str(media['title']))
            return media
    return None
...
def tmdb_search(search_query=None, year=None):
    ...
    tmdb_api_key = cfg.arm_config['TMDB_API_KEY']
    search_results, poster_base, response = tmdb_fetch_results(search_query, year, tmdb_api_key)
    app.logger.debug("TMDB search results - movie - %d results", int(search_results.get('total_results', 0)))
    if 'status_code' in search_results:
        app.logger.error("tmdb_fetch_results failed with status_code %s", int(search_results.get('status_code', 0)))
        return None
    return_results = {}
    if search_results['total_results'] > 0:
        app.logger.debug("tmdb_search - found %d movies", int(search_results['total_results']))
...
def tmdb_process_results(poster_base, return_results, search_results, media_type="movie"):
    ...
    for result in search_results['results']:
        app.logger.debug("Processing TMDB result: %s", str(result.get('title', result.get('name', 'unknown'))))
...
def tmdb_get_imdb(tmdb_id):
    ...
    if 'status_code' in search_results:
        ...
        app.logger.debug("TMDB TV external IDs response keys: %s", [str(k) for k in tv_json.keys()])
